...
# from handlers.open_files import all_admin_id
# from keyboards.admin.panel_group import main_key_group  <- Панель для Админа
        ...
        if new_member.id == dp.bot.id:  # Если бот добавлен в группу
            ...
            if not is_group_id_exists(chat_id):
                ...
                with open('handlers/user_admin_start.txt', 'r', encoding='utf-8') as file:
                    ...
                    if match:
                        admin_id = message.from_user.id # match.group(1)  # ID в чистом виде
